...
int utn_getCadenaWithPoint(char *pAux, int limit, int retry, char* msg, char* msgError)
{
	int isOk = 1;
    char auxString[limit];
    ...
    if(pAux != NULL && msg != NULL && msgError != NULL && limit > 0 && retry >= 0){
        do{
            ...
            if(getCadena(auxString, limit) == 0 && isCharWithPoint(auxString) == 0)
            {
                strncpy(pAux, auxString, limit);
                isOk = 0;
                break;
